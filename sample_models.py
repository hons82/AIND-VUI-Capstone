...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers-1):
        simp_rnn = GRU(units, return_sequences=True, implementation=2, name='rnn'+str(i))(simp_rnn)
        bn_rnn = BatchNormalization()(simp_rnn)
    # DONE: Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
...
def final_model(input_dim, filters, kernel_size, conv_stride, conv_border_mode, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        ...
        simp_rnn = GRU(units, return_sequences=True, implementation=2, name='rnn'+str(i))(dropout)
        # Add batch normalization
        bn_rnn = BatchNormalization()(simp_rnn)
        # Add Dropout
